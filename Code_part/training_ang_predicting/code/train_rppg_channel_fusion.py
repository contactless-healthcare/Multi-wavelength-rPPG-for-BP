...
def ppg_hdf2tfrecord(h5_file, tfrecord_path, samp_idx):
    ...
    with h5py.File(h5_file, 'r') as f:
        ppg_h5 = f.get('/ppg')
        ppg_h5 = np.transpose(ppg_h5, (1, 2, 0))
        ...
        for i in np.nditer(samp_idx):
            ppg = np.array(ppg_h5[i, :, :], dtype=np.float32)
            target = np.array(BP[i, :], dtype=np.float32)
            sub_idx = np.array(subject_idx[i])

            data = \
                {'ppg': _bytes_feature(ppg.tostring()),
...
def read_tfrecord(example, win_len):
        ...
        tfrecord_format = tf.io.parse_single_example(example,
        features={
        'ppg': tf.io.FixedLenFeature([], tf.string),
        'label': tf.io.FixedLenFeature([3], tf.float32)
        })

        ppg_combined = tf.io.decode_raw(tfrecord_format['ppg'], tf.float32)
        ppg_combined = tf.reshape(ppg_combined, [1, win_len, dimension])
        ppg_combined = tf.squeeze(ppg_combined)

        return ppg_combined, (tfrecord_format["label"][0], tfrecord_format["label"][1], tfrecord_format["label"][2])
...
def create_dataset(tfrecords_dir, win_len, batch_size, modus='train'):
    ...
    dataset = dataset.map(partial(read_tfrecord, win_len=win_len),
...
def train_one_subject(idx, architecture,
    ...
    data_in_shape = (win_len, dimension)
    model = get_model(architecture, data_in_shape, UseDerivative=False)
    if idx == 1:
    ...
    if lr is None:
        lr_schedule = CosineDecay(initial_learning_rate=0.1, decay_steps=N_epochs)
...
if __name__ == "__main__":
    ...
    if len(argv) > 1:
        ...
        parser.add_argument('--dimension', type=int, default="3", help="dimensions of input data (channels)")
